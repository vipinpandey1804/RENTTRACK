...
export default function AcceptInvitePage() {
  ...
  const [invite, setInvite] = useState<{ email: string; role: string; organization: string } | null>(null);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!Object.keys(mapped).length) mapped.form = 'Something went wrong. Please try again.';
      setErrors(mapped);
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 px-4">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 text-center">
          ...
          <p className="text-gray-500 text-sm mb-6">
            This invitation link is no longer valid. Please contact your landlord to send a new one.
          </p>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-white/60 p-8">
          ...
          <div className="rounded-xl bg-blue-50 border border-blue-100 p-4 mb-6">
            ...
            <p className="text-sm text-blue-600">
              Joining as <span className="font-medium capitalize">{invite?.role?.replace('_', ' ')}</span> · {invite?.email}
            </p>
          </div>

          <h1 className="text-xl font-bold text-gray-900 mb-1">Create your account</h1>
          <p className="text-sm text-gray-500 mb-6">Complete your details to accept the invitation</p>

          <form onSubmit={onSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <Input label="First name" value={form.first_name} onChange={set('first_name')}
                required autoComplete="given-name" error={errors.first_name} />
            ...
            <Input label="Password" type="password" value={form.password} onChange={set('password')}
              required autoComplete="new-password" hint="Minimum 10 characters" error={errors.password} />
