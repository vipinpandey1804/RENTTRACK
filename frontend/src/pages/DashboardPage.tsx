...
export default function DashboardPage() {
  ...
  const totalUnits =
    properties?.results.reduce((sum, p) => sum + p.unit_count, 0) ?? 0;
  const occupiedUnits =
    properties?.results.reduce((sum, p) => sum + p.occupied_count, 0) ?? 0;
    ...
    <AppShell>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          >
            <div
              className={`inline-flex h-9 w-9 rounded-xl ${bg} items-center justify-center mb-3`}
            >
              {icon}
            </div>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-400 truncate">
                      {b.lease_info.tenant_name || b.lease_info.tenant_email} ·{" "}
                      {b.lease_info.unit_name}
                    </p>
                  </div>
      ...
      {!isLoading && properties?.count === 0 && (
        <div className="rounded-2xl border-2 border-dashed border-gray-200 p-16 text-center mt-4">
          ...
          <p className="text-sm text-gray-500 mb-6">
            Start by adding your first property to manage rentals
          </p>
          <Link
            to="/properties"
