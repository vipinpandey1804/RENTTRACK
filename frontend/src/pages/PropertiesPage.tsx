  ...
  return (
    ...
    >
      ...
      <div className="relative h-44 overflow-hidden">
        ...
        <span className={`absolute top-3 right-3 text-xs font-semibold px-2.5 py-1 rounded-full backdrop-blur-sm bg-white/90 border border-white/60 ${TYPE_BADGE[property.property_type]}`}>
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-4 text-sm">
            <div className="text-center">
              <p className="font-bold text-gray-900 text-lg leading-none">{property.unit_count}</p>
              <p className="text-gray-400 text-xs mt-0.5">units</p>
            </div>
            <div className="text-center">
              <p className="font-bold text-gray-900 text-lg leading-none">{property.occupied_count}</p>
          ...
          <div className="text-right">
            <div className="flex items-center gap-1.5 justify-end mb-1">
              <div className="w-20 h-1.5 rounded-full bg-gray-100">
                <div
                  className={`h-1.5 rounded-full ${occupancy >= 80 ? 'bg-emerald-500' : occupancy >= 40 ? 'bg-blue-500' : 'bg-amber-500'}`}
                  style={{ width: `${occupancy}%` }}
                />
              </div>
              <span className="text-xs text-gray-500 font-medium w-8 text-right">{occupancy}%</span>
            </div>
...
function AddPropertyModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const onSubmit = async (data: PropertyFormData) => {
    try {
      await createProperty.mutateAsync({ ...data, cover_image_file: imageFile ?? undefined } as any);
    ...
    <AppShell>
      ...
      {!isLoading && !isError && data?.results.length === 0 && (
        <div className="rounded-2xl border-2 border-dashed border-gray-200 p-16 text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">Add your first property to start managing rentals</p>
