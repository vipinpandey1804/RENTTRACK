...
import { mockApi, seedAuth, seedTenantAuth, MOCK_BILLS, MOCK_LEASES } from './helpers';
...
test.describe('Tenant Dashboard — routing', () => {
  test('owner role sees landlord DashboardPage (not TenantDashboard)', async ({ page }) => {
    await seedAuth(page);
    ...
    await expect(page.getByText(/your tenancy/i)).not.toBeVisible({ timeout: 3000 });
  });
  ...
  test('tenant role is routed to TenantDashboardPage', async ({ page }) => {
    ...
    await page.goto('/dashboard');
    await expect(page.getByText(/your tenancy/i)).toBeVisible({ timeout: 5000 });
  });
});
...
test.describe('Tenant Dashboard — content', () => {
  ...
  test('shows email not verified banner for unverified tenant', async ({ page }) => {
  ...
  test('all caught up state shown when no outstanding bills', async ({ page }) => {
    await mockApi(page, 'GET', '/billing/bills/', { count: 0, results: [], next: null, previous: null });
    await page.reload();
...
test.describe('Tenant Dashboard — no active lease', () => {
  test('shows no active lease message', async ({ page }) => {
    ...
    await mockApi(page, 'GET', '/billing/bills/', { count: 0, results: [], next: null, previous: null });
    await page.goto('/dashboard');
    await expect(page.getByText(/no active lease found/i)).toBeVisible({ timeout: 5000 });
  });
});
