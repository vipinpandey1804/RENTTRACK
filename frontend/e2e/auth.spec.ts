...
test.describe("Login page", () => {
  ...
  test("shows error on bad credentials", async ({ page }) => {
    ...
    await expect(
      page.getByText(/invalid|incorrect|no active account/i),
...
test.describe("Email verification banner", () => {
  ...
  test("does not show banner for verified owner", async ({ page }) => {
    ...
    await page.goto("/dashboard");
    await expect(page.getByText(/email not verified/i)).not.toBeVisible({
      timeout: 3000,
    });
  });
});
...
test.describe("Protected routes", () => {
  test("redirects unauthenticated user to /login from /dashboard", async ({
    page,
  }) => {
    await page.goto("/dashboard");
  ...
  test("redirects unauthenticated user to /login from /billing", async ({
    page,
  }) => {
  ...
  test("redirects unauthenticated user to /login from /properties", async ({
    page,
  }) => {
